...
void CrrcFault::installMvb(CrrcRicoMvb *crrcMvb)
{
    this->m_CrrcRico_Mvb = crrcMvb;
}
...
void CrrcFault::run()
{
    while(1)
    {
        ...
        foreach (quint32 key, this->FaultTypeHash.keys())
        {
            ...
            if(!m_CrrcRico_Mvb->getportexist(FaultTypeHash[key].PortAddress))
            {
                //not defined ports ;
            }else
            {
                if (FaultTypeHash[key].FaultValid == m_CrrcRico_Mvb->getBool(FaultTypeHash[key].PortAddress, FaultTypeHash[key].ByteoffAddress, FaultTypeHash[key].BitoffAddress))
                {
